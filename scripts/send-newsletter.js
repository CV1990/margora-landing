...
const SOCIAL_LINKS = [
  { label: "LinkedIn", url: "https://www.linkedin.com/company/margora-ac/" },
  { label: "Facebook", url: "https://www.facebook.com/margoraConsultora" },
  { label: "Instagram", url: "https://www.instagram.com/consultoramargora/" },
  { label: "YouTube", url: "https://www.youtube.com/@margora-consult" },
]
...
function buildEmailHtml(post, postUrl) {
  ...
  const socialHtml = SOCIAL_LINKS.map(
    (s) =>
      `<a href="${escapeHtml(s.url)}" style="color: #0ea5e9; text-decoration: none; margin-right: 12px;">${escapeHtml(s.label)}</a>`
  ).join("")
...
<body style="font-family: system-ui, -apple-system, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 24px;">
  <div style="margin-bottom: 24px;">
    <strong style="font-size: 18px; color: #0ea5e9;">Margora</strong>
  </div>
...
async function main() {
  ...
  const subject = `Margora | ${post.title}`
